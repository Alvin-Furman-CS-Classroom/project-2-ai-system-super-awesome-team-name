...
- **Module 5 (RL / personalization):** Full spec is in **README.md** (Module 5 section). **Persistence:** default file `data/user_profile.json` (thresholds + RL state + meta); load must tolerate missing/corrupt JSON; save should be **atomic**. **User outcomes** (feedback vocabulary): `no_spike`, `mild_spike`, `spike`. **Learned knobs** are the same GI/GL limits as Module 2: `safe_gl`, `caution_gl`, `safe_gi`, `caution_gi` (defaults align with `src/module2/safety_rules.py`). **RL:** coarse **state** from Module 3’s `meal_risk_category` and a **risk_score band**; discrete **actions** that nudge one threshold (or `no_op`); **ε-greedy** policy; **terminal Q-learning** (`γ = 0`): `Q(s,a) ← Q(s,a) + α (r − Q(s,a))`. **Rewards:** **safety-first** — stronger penalty for under-predicting spikes than for over-predicting caution. After implementation, **Module 2** must evaluate rules using the loaded thresholds (not unused constructor args); **Module 3** effective-GL **category/score mapping** must use the same **`safe_gl` / `caution_gl`** as Module 2. **Tests:** `unit_tests/module5/`, `integration_tests/module5/`. Source outlines: `src/module5/types.py`, `user_profile.py`, `rl_threshold_adapter.py`, `personalization_service.py`.
